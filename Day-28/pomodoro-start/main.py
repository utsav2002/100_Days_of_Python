...
def reset_timer():
    ...
    time_heading_label.config(text="Timer")
...
def start_timer():
    ...
    if reps % 8 == 0:
        ...
        time_heading_label.config(text="Break", fg=RED)

    elif reps % 2 == 0:
        count_down(short_break_sec)
        time_heading_label.config(text="Break", fg=PINK)

    else:
        count_down(work_sec)
        time_heading_label.config(text="Work", fg=GREEN)
...
# Labels
time_heading_label = Label(text="Timer", fg=GREEN, font=(FONT_NAME, 80, "normal"), highlightthickness=0, bg=YELLOW)
time_heading_label.grid(column=1, row=0)

tick_mark_label = Label(text="", fg=GREEN, bg=YELLOW)
